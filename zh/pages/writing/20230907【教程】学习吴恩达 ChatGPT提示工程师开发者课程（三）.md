# 学习吴恩达 ChatGPT提示工程师开发者课程（三）

开始学习吴恩达的这个提示工程开发者课程。欢迎跟我一起学习，课程地址在这里：https://www.deeplearning.ai/short-courses/chatgpt-prompt-engineering-for-developers/

## 课程内容简介

这次的课程内容比较简单，7分钟左右，吴恩达举例讲了进行文本摘要时的几种Prompt写法：

1. **文本摘要**:
   - 输入了一个关于熊猫毛绒玩具的产品评论。
   - 重点是使用摘要来为电子商务网站生成一个简短的产品评论。
2. **给摘要一些限制**:
   - 按字数、句子或字符进行限制。
   - 比如提示模型使用不超过30个词的摘要。
3. **作摘要时聚焦某些内容**:
   - 根据评论的特定方面进行摘要，如运输和交货或价格和价值。
   - 比如提示模型关注与配送和交货相关的内容，以及与价格和感知价值相关的内容。
4. **“提取” vs “摘要”**:
   - 除了摘要之外，还尝试了“提取”相关信息的方法，特别是针对运输部门的反馈。
   - 比如提示模型直接从评论中提取与运输和交货相关的信息。
5. **多个产品评论的摘要**:
   - 输入了四个不同产品的评论：熊猫玩具、站立灯、电动牙刷和搅拌机。
   - 用了个For循环，对每个评论进行了摘要，限制为最多20个词。
   - 结果显示了每个评论的简短摘要。
...
> 给我输出不超过50个字的摘要，重点是例子

他说：

> **摘要**： 课程主要教授如何使用模型摘要电商产品评论。示例包括熊猫玩具、站立灯、电动牙刷和搅拌机的评论。

我又说：

> 给我输出不超过50个字的课程摘要，覆盖重点内容

这次精准了不少：

> **课程教授如何为电商网站摘要产品评论，通过不同焦点（如送货、价格）和限制（如字数）来迭代改进提示。**
...
From the review below, delimited by triple quotes \
extract the information relevant to price. Limit to 10 words. 
...
response = get_completion(prompt)
print(response)
"a bit small for what I paid though. other options that are bigger for the same pric
```
